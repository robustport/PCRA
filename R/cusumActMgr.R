...
# Declare variables referenced via non-standard evaluation (ggplot2 aes(), dplyr)
# so that R CMD check does not flag them as undefined global variables.
utils::globalVariables(c(
  "Index", "Active", "ExcessVol", "Portfolio", "Benchmark", "AnnualMovingAverage",
  "Cusum_Log_AR", "CusumIR", "LRTest",
  "Ray-3", "Ray-2", "Ray-1", "Ray0", "Ray+1", "Ray+2", "Ray+3"
))
...
#' @name chartCusum
#' @title chartCusum Plot the output of cusumActMgr
#' @description Produce the two diagnostic chart pages from a \code{cusumActMgr} result object.
#' @param results_obj A \code{list} returned by \code{\link{cusumActMgr}}
#' @param digits Number of significant digits used when printing. Default = 3
#' @param select_option Integer (or integer vector) selecting which page(s) to plot: 1 or 2
#' @param print_to_screen Logical, draw the page(s) on the active device. Default \code{TRUE}
#' @param print_to_png Logical, write the page(s) to a PNG file. Default \code{TRUE}
#' @param print_to_pdf Logical, write the page(s) to a PDF file. Default \code{FALSE}
#' @param png_fn Character file name for PNG output
#' @param pdf_fn Character file name for PDF output
#' @param ... Further arguments (currently unused)
#' @return Called for its side effect of drawing or saving charts; returns \code{NULL} invisibly.
#' @export
chartCusum <- function(results_obj, digits = 3, select_option = NULL,
  ...
  results <- results_obj
  ptfName <- results_obj$portfolioName
  bmkName <- results_obj$benchmarkName
  df_xts  <- results_obj$returns
...
#' A named \code{list} of \code{xts} objects and summary vectors is returned.
#'
#' @details
...
#' \item{Annualized_Cusum_Log_AR}{Vector of the CUSUM of the annualized log active returns}
#' \item{Log_AR_Slopes}{Numeric vector of the seven protractor ray slopes for the log active return plot}
#' \item{IR_Slopes}{Numeric vector of the seven protractor ray slopes for the information ratio plot}
#' \item{portfolioName}{The portfolio name passed in by the user}
...
#' @examples
#' \dontrun{
#' # Data Preprocessing: convert BXM (CBOE S&P 500 Buywrite Index) 
#' # and SPTR (S&P 500 Total Return) levels to returns, remove NAs
#' df <- CboeOptionStrategies[, c("Date", "BXM", "SPTR")]
